...
- Có thể thực hiện các phép tính trong dấu ngoặc biến, chẳng hạn như `{1 * (2 + 3)}`.
- Dấu ngoặc biến không còn có thể được lồng vào nhau.
- Mục tiêu thực thi công thức có thể được đặt thành `tất cả người chơi trong thế giới`.
...
- Đã thêm một hành động:
  - `Kích hoạt một kích hoạt tùy chỉnh`.
- Thêm lệnh `/mineflow recipe execute`.

### API

- Đã xóa các lớp không dùng nữa trong phiên bản 1.3.1
- Thay đổi `FlowItem::getEditForm`.
  - Sử dụng `FlowItem::getEditFormElement` thay thế.
- Thay đổi `FlowItem::parseFormData`.
  - Đối số chỉ được đưa cho phần tử của `FlowItem::getEditFormElement` đã được nhập.
...
- Xóa `FlowItem::sendCustomMenu`.
  - Sử dụng `FlowItem::getCustomMenuButtons` thay thế.
- Xóa `EventTriggerList`.
  - Sử dụng `EventManager` (`Main::getEventManager()`) thay thế
    - `EventTriggerList::add()` -> `EventManager::addTrigger`
- Tách `EventManager::setEventEnabled` thành `EventManager::enable` và `EventManager::disable`.
...
- Sửa lỗi mà các biến trong giá trị của `SetConfigData` không được thay thế.
- Sửa sai danh mục của `Tồn tại tệp cấu hình` và `Tồn tại dữ liệu trong tệp cấu hình`.
...
- Sửa lỗi khi thêm công thức [#159](https://github.com/aieuo/Mineflow/issues/159)
...
- Sửa [#163](https://github.com/aieuo/Mineflow/issues/163)
- Sửa [#170](https://github.com/aieuo/Mineflow/issues/170)
