...
- Giờ đây có thể thực hiện các phép tính trong dấu ngoặc biến, ví dụ: `{1 * (2 + 3)}`.
- Dấu ngoặc biến không còn có thể được lồng vào nhau.
- Mục tiêu thực thi công thức giờ đây có thể được đặt thành `tất cả người chơi trong thế giới`.
...
- Đã thay đổi `FlowItem::parseFormData`.
  - Đối số chỉ được cung cấp cho phần tử của `FlowItem::getEditFormElement` đã được nhập.
...
- Đã sửa lỗi biến trong giá trị của `SetConfigData` không được thay thế.
- Đã sửa danh mục sai của `Tồn tại tệp cấu hình` và `Tồn tại dữ liệu trong tệp cấu hình`.
...
- Đã sửa sự cố khi thêm công thức [#159](https://github.com/aieuo/Mineflow/issues/159)
